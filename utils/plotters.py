...
# --- PHASE 2 & 3: New & Enhanced Analytical Plots ---
...
def plot_historical_control_chart(df, cqa: str, date_range: tuple):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df_filtered['injection_time'], y=df_filtered[cqa], mode='lines+markers', name=cqa, marker_color=VERTEX_COLORS['blue']))
    ...
    fig.update_layout(title=f"<b>Historical Control Chart (I-Chart) for {cqa}</b>", xaxis_title="Date", yaxis_title="Value", height=350, showlegend=False)
    return fig
...
def plot_process_capability(df, cqa: str, lsl: float, usl: float):
    ...
    fig = px.histogram(df, x=cqa, nbins=30, title=f"<b>Process Capability for {cqa} | Cpk: {cpk:.2f}</b>", color_discrete_sequence=[VERTEX_COLORS['lightblue']])
...
def plot_stability_trend(df, assay: str, spec_limits: dict):
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df['Timepoint (Months)'], y=df[assay], mode='lines+markers', name=assay, marker_color=VERTEX_COLORS['blue']))
    ...
    fig.update_layout(title=f"<b>Stability Trend for {assay}</b>", xaxis_title="Timepoint (Months)", yaxis_title="Value", height=400, legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1))
    return fig

# --- Preserved Original Plotting Functions ---
...
def plot_pareto_chart(df):
    ...
    fig.add_trace(go.Scatter(x=df_sorted['Error Type'], y=df_sorted['cumulative_percentage'], name='Cumulative %', yaxis='y2', mode='lines+markers', line=dict(color=VERTEX_COLORS['blue'])))
