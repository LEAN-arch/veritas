...
from utils.analysis_engine import calculate_stability_projection

def render_page():
    """
    Renders the Stability Program Dashboard.
    """
    ...
    if stability_df.empty or not app_config or not db_connection:
    ...
    if not filtered_df.empty:
        ...
        with col1:
            ...
            if assay_purity in spec_limits_config and assay_purity in filtered_df.columns:
                st.plotly_chart(
                    plot_stability_trend(filtered_df, assay_purity, spec_limits_config[assay_purity]),
                    use_container_width=True
                )
                # Call analysis engine for projection
                lsl = spec_limits_config[assay_purity]['LSL']
                projection = calculate_stability_projection(filtered_df, 'Timepoint (Months)', assay_purity, lsl)
        ...
        with col2:
            ...
            if assay_impurity in spec_limits_config and assay_impurity in filtered_df.columns:
                st.plotly_chart(
                    plot_stability_trend(filtered_df, assay_impurity, spec_limits_config[assay_impurity]),
                    use_container_width=True
                )
                # Call analysis engine for projection
                usl = spec_limits_config[assay_impurity]['USL']
                projection = calculate_stability_projection(filtered_df, 'Timepoint (Months)', assay_impurity, usl)
                if projection and 'months_to_spec' in projection:
                     st.metric(
